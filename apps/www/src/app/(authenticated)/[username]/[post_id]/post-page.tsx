      ...
      {postQuery.isLoading ? (
        ...
        <div className="flex h-[calc(100vh-10rem)] flex-col gap-y-2 sm:h-[calc(100vh-4.60rem)]">
          <div className="flex flex-col gap-y-4 p-4">
            ...
            {/* <Comments post={postQuery.data.post} /> */}
...
function PostFooter({
  ...
  userId: string;
  post: Post & { likes: Like[]; comments: CommentType[] };
}) {
...
function CommentForm({ post }: { post: Post & { comments: CommentType[] } }) {
  const context = api.useContext();
  ...
  useEffect(() => {
    searchParams.has("comment") ? setIsFocused(true) : setIsFocused(false);
  }, [searchParams]);

  useEffect(() => {
    isFocused
      ? router.push(
          `/${params.username as string}/${params.post_id as string}?comment`,
        )
      : router.push(
          `/${params.username as string}/${params.post_id as string}`,
        );
